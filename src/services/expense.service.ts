...
async function getExpenses(accessToken: string): Promise<ExpenseDTO[]> {
    ...
    // All expenses belong to the authenticated user (RLS), so profile_name is the same for every row.
    const { data: profile } = await db.from('profiles').select('name').single()
